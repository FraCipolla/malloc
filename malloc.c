...
static void init()
{
    ...
    for (int len = 0; len < tiny_alloc; len += TINY) {
        if (len == tiny_alloc - TINY) {
            memcpy(
                &small_ptr[len],
                &(t_header){.free=true, .size=TINY, .next=nullptr, .index=index++},
                sizeof(t_header)
                );
            break;
        }
        memcpy(
            &small_ptr[len],
            &(t_header){.free=true, .size=TINY, .next=(void *)&small_ptr[len + TINY], .index=index++},
            sizeof(t_header)
            );
    }
    ...
    for (int len = 0; len < small_alloc; len += SMALL) {
        ...
        memcpy(
            &medium_ptr[len],
            &(t_header){.free=true, .size=SMALL, .next=(void *)&medium_ptr[len + SMALL], .index=index++},
            sizeof(t_header)
            );
    }
}
...
void free(void *ptr)
{
    ...
    ptr -= sizeof(t_header);
    t_header *header_cast = (t_header *)ptr;
    header_cast->free = true;
    t_header *free_blocks = (t_header *)g_blocks.to_free;
    if (!free_blocks) {
        free_blocks = header_cast;
    } else {
        while (free_blocks->next) {
            free_blocks = free_blocks->next;
        }
        free_blocks->next = header_cast;
    }
}